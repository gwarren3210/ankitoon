...
describe('textGrouper', () => {
  describe('groupOcrIntoLines', () => {
    it('returns empty array for empty input', async () => {
      const result = await groupOcrIntoLines([])
      expect(result).toEqual([])
    })

    it('groups single item into one line', async () => {
      const input: OcrResult[] = [
        { text: 'hello', bbox: { x: 10, y: 20, width: 50, height: 30 } }
      ]

      const result = await groupOcrIntoLines(input)
    ...
    it('groups items within vertical threshold', async () => {
      const input: OcrResult[] = [
      ...
      const result = await groupOcrIntoLines(input, 50)
    ...
    it('separates items beyond vertical threshold', async () => {
      const input: OcrResult[] = [
    ...
    it('creates multiple groups for multiple speech bubbles', async () => {
      const input: OcrResult[] = [
        // First bubble
      ...
      const result = await groupOcrIntoLines(input, 100)
    ...
    it('respects custom threshold', async () => {
      ...
      const result = await groupOcrIntoLines(input, 20)
      expect(result).toHaveLength(2)

      // Should be grouped with threshold of 50
      const result2 = await groupOcrIntoLines(input, 50)
      expect(result2).toHaveLength(1)
    })
  })

  describe('text ordering', () => {
    it('sorts horizontally for same-line items', async () => {
      const input: OcrResult[] = [
    ...
    it('handles slight vertical variance on same line', async () => {
      const input: OcrResult[] = [
    ...
    it('sorts vertically then horizontally for multi-line groups', async () => {
      const input: OcrResult[] = [
  ...
  describe('combined bounding box', () => {
    it('calculates bbox encompassing all items', async () => {
      const input: OcrResult[] = [
    ...
    it('handles single item bbox correctly', async () => {
      const input: OcrResult[] = [
    ...
    it('each group has its own bounding box', async () => {
      const input: OcrResult[] = [
  ...
  describe('edge cases', () => {
    ...
    it('handles items at exact threshold distance', async () => {
      const input: OcrResult[] = [
        { text: 'A', bbox: { x: 10, y: 100, width: 20, height: 20 } },
    ...
    it('handles items just beyond threshold', async () => {
      const input: OcrResult[] = [
        { text: 'A', bbox: { x: 10, y: 100, width: 20, height: 20 } },
  ...
  describe('integration with real OCR data', () => {
    ...
    it('groups Solo Leveling OCR data correctly', async () => {
      const result = await groupOcrIntoLines(ocrInput, 100)

      expect(result).toHaveLength(expectedOutput.length)
    })

    it('combines first speech bubble text correctly', async () => {
      const result = await groupOcrIntoLines(ocrInput, 100)
    ...
    it('groups E-rank hunter bubble correctly', async () => {
      const result = await groupOcrIntoLines(ocrInput, 100)
    ...
    it('maintains correct Y ordering of groups', async () => {
      const result = await groupOcrIntoLines(ocrInput, 100)
    ...
    it('produces bounding boxes that encompass all words', async () => {
      const result = await groupOcrIntoLines(ocrInput, 100)
