-- Fix get_study_cards RPC to fetch settings from profile and properly select due cards
-- Changes:
-- 1. Removes p_max_new_cards and p_max_total_cards parameters
-- 2. Fetches these values from profiles table instead
-- 3. Fixes bug where only new cards were selected (due cards are now included properly)
-- 4. Ensures profile columns have proper defaults and updates existing NULL values

-- Update existing NULL values to defaults
UPDATE profiles
SET max_new_cards = 10
WHERE max_new_cards IS NULL;

UPDATE profiles
SET max_total_cards = 30
WHERE max_total_cards IS NULL;

-- Ensure column defaults are set (in case they weren't set correctly)
ALTER TABLE profiles
ALTER COLUMN max_new_cards SET DEFAULT 10,
ALTER COLUMN max_total_cards SET DEFAULT 30;
