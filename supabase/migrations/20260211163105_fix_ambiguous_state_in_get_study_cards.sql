-- Migration: Fix ambiguous column reference in get_study_cards RPC
-- Problem: "state" was ambiguous between RETURNS TABLE column and CTE column
-- Fix: Add table alias 'acc' and qualify all column references in CTEs
...
BEGIN
  ...
  new_cards AS (
    SELECT * FROM all_chapter_cards acc
    WHERE acc.srs_state = 'New'::srs_state
    ORDER BY acc.srs_created_at ASC
    LIMIT v_max_new_cards
  ),
  due_cards AS (
    SELECT * FROM all_chapter_cards acc
    WHERE acc.due <= v_now
      AND acc.srs_state != 'New'::srs_state
    ORDER BY acc.due ASC
  )
  SELECT
  ...
  ORDER BY
    CASE WHEN combined.srs_state = 'New'::srs_state THEN 0 ELSE 1 END,
    combined.due ASC
