-- Migration: Add timestamp fields to get_user_library_decks RPC
-- Purpose: Fix type mismatch where TypeScript expects created_at/updated_at
-- but RPC was not returning these fields (hardcoded to null in transformation)

-- Drop existing function first (can't change return type with CREATE OR REPLACE)
DROP FUNCTION IF EXISTS get_user_library_decks(UUID);

CREATE FUNCTION get_user_library_decks(p_user_id UUID)
RETURNS TABLE (
  -- Chapter fields
  chapter_id UUID,
  ...
  -- Progress fields (ADDED: created_at, updated_at)
  progress_id UUID,
  ...
  progress_created_at TIMESTAMP WITH TIME ZONE,
...
BEGIN
  ...
  SELECT
    ...
    s.created_at,
    s.updated_at,
    -- Progress (ADDED: up.created_at, up.updated_at)
    up.id,
    up.accuracy,
